...
MODEL_PATH = "vocal_model_loso2.h5"
SR = 22050
...
SONG_DB = {
    ...
    "F": ["All of Me - John Legend (sering di F)", "Someone Like You - Adele (sering di F versi piano)"],
...
def snap_do_to_nearest_C_below(median_midi: int) -> int:
    """Pilih Do sebagai C terdekat di bawah nada median."""
    pc = median_midi % 12
    do_midi = median_midi - pc
...
def build_major_scale_one_octave(do_midi: int):
    ...
    return [librosa.midi_to_note(do_midi + i) for i in intervals]
...
def detect_comfort_note(y):
    ...
    if med is None:
        return None, None
    return med, librosa.hz_to_note(med)
...
def score_resonance_for_note_segment(y_note, model):
    ...
    return None if len(scores) == 0 else float(np.median(scores))
...
# =========================
def analyze_scale_test(y, model, target_notes, note_duration, res_threshold):
    """
    Segmentasi berbasis waktu: 1 nada = note_duration detik.
    """
    ...
    for i, target in enumerate(target_notes):
        ...
        _, note_detected = detect_note_in_segment(seg)
        ...
        if note_detected != "N/A":
            ...
            if cents is not None and cents <= 150:
                pitch_ok = True
...
# =========================
def get_musical_recommendation_from_scale(df, res_threshold):
    dfv = df.copy()
    dfv["Skor AI"] = pd.to_numeric(dfv["Skor AI"], errors="coerce")
...
def main():
    ...
    # Session defaults
    if "res_threshold" not in st.session_state:
        st.session_state.res_threshold = 0.55
    if "note_duration" not in st.session_state:
        st.session_state.note_duration = 1.0
    ...
    st.session_state.res_threshold = st.sidebar.slider(
        "RES_THRESHOLD", 0.40, 0.80, float(st.session_state.res_threshold), 0.01
    )

    st.sidebar.markdown("---")
    st.sidebar.write("Durasi per nada (detik)")
    opts = [0.75, 1.0, 1.25]
    idx = opts.index(st.session_state.note_duration) if st.session_state.note_duration in opts else 1
    st.session_state.note_duration = st.sidebar.selectbox("NOTE_DURATION", opts, index=idx)
    ...
    with col2:
        ...
        if mode.startswith("2)"):
            manual_do = st.selectbox(
                "Pilih Do awal (manual)",
                ["C3", "C4", "D3", "D4", "E3", "F3", "G3", "A3"],
                index=0
            )
        ...
        if st.button("🚀 MULAI ANALISIS SCALE", type="primary", disabled=not can_run):
            with st.spinner("Menganalisis pitch + kualitas resonansi per nada..."):
                df = analyze_scale_test(
                    scale_audio,
                    model,
                    st.session_state.target_scale,
                    note_duration=float(st.session_state.note_duration),
                    res_threshold=float(st.session_state.res_threshold)
                )

                ceiling, key_name, _, key_pc = get_musical_recommendation_from_scale(
                    df,
                    res_threshold=float(st.session_state.res_threshold)
                )
                ...
                with c1:
                    st.metric("Vocal Ceiling (Atap Aman)", ceiling if ceiling else "Tidak terdeteksi")
                ...
                with c2:
                    if key_name is None:
                        ...
                        st.caption("Logika: Root = Ceiling − 7 semitone (Perfect Fifth).")
                ...
                if not df_plot.empty:
                    ...
                    for _, row in df_plot.iterrows():
                        if row["Pitch OK"] != True:
                            colors.append("#95a5a6")  # gray invalid pitch
                        else:
                            colors.append("#2ecc71" if row["Kualitas"] == "Resonant" else "#e74c3c")

                    ax.bar(df_plot["Target Nada"], df_plot["Skor AI"], color=colors)
                    ax.axhline(st.session_state.res_threshold, color="black", linestyle="--", alpha=0.6, label="Threshold")

                    if ceiling is not None and key_name is not None:
                        ax.set_title(f"Ceiling: {ceiling} | Recommended Key: {key_name}")
                        ax.annotate(
                            f"CEILING {ceiling}",
                            xy=(ceiling, st.session_state.res_threshold),
                            xytext=(ceiling, 0.95),
                ...
                if key_pc is None:
                    ...
                    if len(songs) == 0:
                        st.warning(f"Belum ada database lagu untuk key {key_pc}. Tambah di SONG_DB.")
                    else:
                        st.write(f"Contoh lagu yang sering dikenal (atau sering dimainkan) di **{key_pc}**:")
                        for s in songs[:5]:
                            st.write(f"- {s}")
